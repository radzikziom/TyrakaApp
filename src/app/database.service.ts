...
export class DatabaseService {
  ...
  constructor(
    ...
    this.serverAddress = 'https://tyraka.herokuapp.com/api';
  ...
  // Engines CRUD ---------------------------------------------------------------------
  loadEngines() {
    this.http.get<Engine[]>(this.serverAddress + '/engines').subscribe(
      data => {
  ...
  loadParts() {
    this.http.get<Body[]>(this.serverAddress + '/bodys').subscribe(
      data => {
  ...
  loadTests() {
    this.http.get<Test[]>(this.serverAddress + '/tests').subscribe(
      data => {
  ...
  createTest(test: Test, results: Result[]) {
    let testId;
    this.http
      .post<Test>(this.serverAddress + '/test', test)
      .subscribe(response => {
        testId = response;
        this.http
          .post(this.serverAddress + '/test/' + testId, { results: results })
          .subscribe();
      });
  ...
  loadGraph(id) {
    this.http
      .get<Result[]>(this.serverAddress + '/testResults/' + id)
      .subscribe(data => {
        let temp = [];
        let thrust = [];
        let time = [];
